...
void print_matrix(const int &m, const int &n, const cuDoubleComplex *A, const int &lda) {
    ...
    void matmul_strided_batched(int m, int n, int k, cuDoubleComplex *A, 
        cuDoubleComplex *B, cuDoubleComplex *C, int batch_count, int nloop)
    {
        ...
        for (int i=1;i<=nstep;i++){
            ...
            printf("batchsize: %d, nloop = %d, CUDA event time: %gs, avg time per matmul : %g \n", batch_size, nloop,elapsed_time/1000.0,elapsed_time/1000.0/double(batch_size*nloop));
        }
